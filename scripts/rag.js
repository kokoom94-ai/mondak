...
async function buildCorpus(needTour) {
  ...
  console.log("코퍼스 구축:", corpus.length, "건 (지원+조례+관광+기관+FAQ+관광통계+의회+교육+학교+공약+뉴스)");
  return corpus;
}
...
const TOURISM_RE = /맛집|관광지|카페|오름|해변|해수욕장|여행|가볼|먹을|볼거리|명소|드라이브|숙소|흑돼지|해산물|올레|박물관|미술관|체험|근처|주변|여행지|놀거리|먹거리|디저트|브런치/;
...
function prefilter(question, corpus, N) {
  ...
  for (const doc of corpus) {
    ...
    for (const t of qs) {
      ...
      if (titleLc.includes(tl)) sc += (t.length >= 3 ? 4 : 2);
      else if (hay.includes(tl)) sc += (t.length >= 3 ? 2 : 1);
    }
    // 역방향: 제목의 의미있는 단어가 질문에 (복합어 대응)
    for (const w of titleLc.split(/[\s·,]+/)) {
      if (w.length >= 2 && qStr.includes(w)) sc += 1;
    }
