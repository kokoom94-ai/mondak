...
def main():
    ...
    # 카페 글은 작성일이 없어 수집일을 쓴다(date_est). 하루에 몰려 보이므로 일별 추이에서는 뺀다.
    # (30일 집계·분야 순위에는 그대로 들어간다)
    m30d=[x for x in m30 if not x.get("date_est")]
    d30=[]
    for dd in sorted({x["date"] for x in m30d if x.get("date")}):
        v=[x for x in m30d if x.get("date")==dd]
        if not v: continue
    ...
    out={"meta":{"updated":NOW.strftime("%Y-%m-%d %H:%M"),
         ...
         "date_est_n":sum(1 for x in items if x.get("date_est")),
         "date_est_note":"카페 글은 작성일이 제공되지 않아 수집일을 날짜로 씁니다. 30일 집계에는 포함하되 일별 추이에는 넣지 않습니다.",
      ...
      "daily":[{"date":dd,"total":len(v),
               ...
               for dd,v in ((dd,[x for x in cur if x.get("date")==dd and not x.get("date_est")])
                            for dd in sorted({x["date"] for x in cur if not x.get("date_est")})) if v],
