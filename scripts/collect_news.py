...
RULES=[
 ...
 ("1차산업",r"감귤|만감류|한라봉|천혜향|농가|농업|어업|어민|해녀|축산|한우|흑우|월동무|당근|메밀|딸기|가뭄|조업|수산|양식|품종|노지|하우스|재배|과수|출하|어가|밭작물|농산물|귀농|귀촌|농지|직불금|양돈|마사회|구제역|백신|풋귤|유제품|농단협|경관보전|국립공원|마을만들기|제주마|밭담"),
 ...
 ("문화/관광/스포츠",r"관광|여행|크루즈|올레길|축제|페스티벌|공연|전시|문화|미술|박물관|콘서트|영화|호텔|리조트|방문객|워케이션|면세|MICE|스포츠|체육|선수|경기|리그|마라톤|대회|감독|구단|제주SK|아시안게임|올림픽|월드컵|비엔날레|음악제|리사이틀|신화|문인협회|신인문학상|국가유산|관아|만장굴|비자림|테라피|그림책|개인전|명상|항공좌석|항공편|항공사|항공이동권|e스포츠|피아니스트|바이올린|화가|래퍼|가볼 만한|여정|힐링|설문대할망|열쇠"),
...
OTHER_REGION=r"울산|부산|대구|인천|광주|대전|세종|수원|성남|용인|고양|창원|청주|천안|전주|포항|김해|경기|강원|충청|충북|충남|전라|전북|전남|경상|경북|경남"
# 사명 속 '제주'는 관련성 증거로 안 침 — 진짜 제주 키워드가 따로 있어야 통과
BRAND=r"제주항공|제주은행|제주유나이티드|제주드림타워"
def jeju_ok(t):
    clean=re.sub(BRAND,"",t)
    if re.search(OTHER_REGION,t) and not re.search(JEJU,clean): return False  # 타지역 무조건 제외
