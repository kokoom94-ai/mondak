...
def classify(t):
    # ── 선처리: 순서 규칙만으로는 갈리지 않는 강한 신호를 먼저 확정한다.
    ...
    if re.search(r"국과수|부검|시신|숨진 채|사망 사고|살해|피살|실종자|변사|"
        ...
        return "사건사고/기타"
    # 2) 봉사·나눔·후원 — '소방대·적십자'가 주체여도 내용은 복지
    if re.search(r"봉사|나눔|후원|기부|성금|위문|돌봄|요양원|경로당|무료 급식|"
                 r"어르신|노인|장애인|취약계층|저소득|한부모|결식", t):
        return "복지/사회서비스"
    # 3) 해녀 = 문화유산
    if re.search(r"해녀|잠녀", t): return "문화/관광/스포츠"
    # 4) 학교명(○○초·○○중·○○고) 또는 학생 활동 = 교육
    if re.search(r"[가-힣]{2,}(초|중|고)등?학?교|[가-힣]{2,}초 |[가-힣]{2,}중 |[가-힣]{2,}고 |"
                 r"학생|교사|교직원|동아리|오케스트라|학예|진로|방과후|급식실", t):
        return "교육/청년"
    # 5) 화재·소방 (봉사가 아닌 실제 화재)
    if re.search(r"소방|화재|폭발|산불", t): return "안전/민원행정"
    # 5-2) 항공·해운 노선 — 관광 접근성 소식
    if re.search(r"노선|취항|증편|감편|운항|직항|결항|여객선|크루즈 기항", t):
        return "문화/관광/스포츠"
    # 6) 도·시 정책 발표 — '모집·공모·시행·지원' 등 행정 행위
